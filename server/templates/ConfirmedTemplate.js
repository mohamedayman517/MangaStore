...
class ConfirmedTemplate {
  ...
  getTemplate() {
    ...
    const utm = { source: "email", medium: "transactional", campaign: "order_confirmed" };
    ...
    const cardContent = `
      <h1 style="margin:0 0 12px;font-size:24px;line-height:1.2;color:#333;font-weight:700;">Your order is confirmed</h1>
      <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#333;">We will notify you once your order ships. Thanks for your purchase!</p>
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-radius:10px;padding:14px 16px;">
        <tr>
          <td>
            ...
            <div style="margin-top:6px;color:#666;font-size:13px;">Order #: ${this.orderData.orderId || "-"}</div>
            <div style="margin-top:4px;color:#666;font-size:13px;">Placed on: ${this.orderData.placedDate || "-"}</div>
            <div style="margin-top:4px;color:#666;font-size:13px;">Payment: ${this.orderData.paymentMethod || "-"}</div>
          </td>
        </tr>
      </table>
    ...
    const unified = new UnifiedBaseTemplate({
      ...
      previewText: `Order #${this.orderData.orderId || ""} confirmed`,
      brand,
      links,
      utm,
    });
    ...
    return unified.render(cardContent, {
      ...
      plaintextFallback: `Order #${this.orderData.orderId || ""} confirmed. Total ${total} ${currency}.`,
    });
  }
}
