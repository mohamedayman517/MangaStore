...
class DeliveredTemplate {
  ...
  getTemplate() {
    ...
    const utm = { source: "email", medium: "transactional", campaign: "delivery_confirmation" };
    ...
    const reviewUrl = `${baseUrl}/profile/orders/${this.orderData.orderId || ""}`;

    const cardContent = `
      <h1 style="margin:0 0 12px;font-size:24px;line-height:1.2;color:#333;font-weight:700;">Delivered — hope you enjoy!</h1>
      <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#333;">Your order <strong>#${this.orderData.orderId || ""}</strong> was delivered. We'd love your feedback.</p>
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border-radius:10px;padding:14px 16px;">
        <tr>
          <td>
            ...
            <div style="margin-top:6px;color:#666;font-size:13px;">Payment: ${this.orderData.paymentMethod || "-"}</div>
          </td>
        </tr>
      </table>
      <table role="presentation" cellspacing="0" cellpadding="0" align="center" style="margin:14px auto 0;">
        <tr>
          <td style="border-radius:8px;overflow:hidden;">
            <a href="${reviewUrl}" style="display:inline-block;text-decoration:none;padding:12px 20px;background:#fea500;color:#ffffff;font-weight:700;border-radius:8px;font-size:16px;min-height:44px;line-height:20px;">Rate your order</a>
          </td>
        </tr>
      </table>
    `;
    ...
    const unified = new UnifiedBaseTemplate({
      ...
      previewText: `Order #${this.orderData.orderId || ""} delivered — please leave a review`,
      brand,
      links,
      utm,
    });
    ...
    return unified.render(cardContent, {
      ...
      plaintextFallback: `Order #${this.orderData.orderId || ""} delivered. Review at ${reviewUrl}`,
    });
  }
}
